...
MPID_Control::MPID_Control()
{
    ...
    cmd_sub = nh_.subscribe(s,1, &MPID_Control::InputCallback, this);
    params.param<std::string>("odometry_topic", s, "odometry/prediction");
    odo_sub = nh_.subscribe(s,1, &MPID_Control::OdoCallback, this, ros::TransportHints().tcpNoDelay());
    ping_sub = nh_.subscribe("ardrone/ping",1, &MPID_Control::PingCallback, this);
    ...
    f = boost::bind(&MPID_Control::dynamic_reconfigure_callback,this, _1, _2);
...
void MPID_Control::dynamic_reconfigure_callback(velocity_control::dynamic_param_configConfig &config, uint32_t level)
{
    if (first_reconfig)
    {
...
void MPID_Control::InputCallback(const geometry_msgs::Twist& cmd_in)
{
    // reference velocity
    command = cmd_in;
}

void MPID_Control::OdoCallback(const nav_msgs::Odometry& odo_msg)
{
...
void MPID_Control::pid_control()
{
    ...
    double beta = 0.9; // 0.9
    Control(0,0) = beta*command.linear.x - vel_xy(0);
    Control(0,1)= beta*command.linear.y - vel_xy(1);
    ...
    error(0) = (filtered_error(0) - old_error_xy(0))/dt.toSec();
    error(1) = (filtered_error(1) - old_error_xy(1))/dt.toSec();
    ...
    Control(2,0) = std::min(max(0), std::max(-max(0), error(0)));
    Control(2,1) = std::min(max(1), std::max(-max(1), error(1)));
...
//    }
    ...
    // Based on u(k)
    control_output_pid.linear.x = std::min(max(0), std::max(-max(0), control_output.linear.x));
    control_output_pid.linear.y = std::min(max(1), std::max(-max(1), control_output.linear.y));
    ...
    ROS_INFO("Derivatitve Error: %f , Error_vel: %f", error(0), error_vel(0));
...
void MPID_Control::set_hover(void)
{
    ROS_INFO("HOVER");
...
void MPID_Control::i_term_set(double &i_value, double error, double vel, double old_vel, double wind_up, double dt)
{
